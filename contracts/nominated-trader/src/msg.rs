...
#[cw_serde]
pub enum ExecuteMsg {
    /// Collects and swaps fee tokens to PALOMA after sending the Swap msgs as SubMsgs
    /// This call is restricted to the currently nominated trader which can perform fee collections and swaps
    Collect {
        /// The nominated assets to swap to PALOMA
        assets: Vec<AssetWithLimit>,
    },
    /// Add or remove route definitions for tokens used to swap specific fee tokens to PALOMA
    ...
    /// Allows the owner to spend the contract's PALOMA balance. The trader contract will not be able to spend the PALOMA but can trade other assets to it.
...
/// This struct holds parameters to help with swapping a specific amount of a fee token to PALOMA.
#[cw_serde]
