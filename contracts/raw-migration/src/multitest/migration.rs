...
fn migrate_raw_with_native() {
    ...
    let grain_token = suite.grain_cw20_token.clone();

    // mint PALOMA tokens for migration contract
    ...
    let raw_to_grain_exchange_rate = Decimal::percent(200);
    suite
    ...
    // Check the balance of PALOMA token instead, it should have been swapped at a 2-1 rate (200%)
    let palomadex = cw20::Cw20Contract(grain_token.clone())
        .balance(&suite.app.wrap(), suite.palomadex_pair_contract.clone())
        .unwrap();
    let expected = liquidity_cw20 * 4 / 5;
    assert_eq!(palomadex.u128(), expected * 2);
    ...
    let pool_info = suite.query_palomadex_pool();
    assert_eq!(
        pool_info.assets,
        vec![
            AssetValidated {
            ...
            AssetValidated {
                info: AssetInfoValidated::Token(grain_token.clone()),
...
fn migration_two_cw20() {
    ...
    // Grin given for RAW tokens
    let palomadex = cw20::Cw20Contract(grain_token.clone())
        .balance(&suite.app.wrap(), suite.palomadex_pair_contract.clone())
        .unwrap();
    let expected = Uint128::from(liquidity * 4 / 5) * raw_to_grain_exchange_rate;
    assert_eq!(palomadex, expected);
